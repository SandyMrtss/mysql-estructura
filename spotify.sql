...
CREATE TABLE IF NOT EXISTS `spotify`.`users_premium` (
  ...
  PRIMARY KEY (`id_user`),
  INDEX `fk_users_premium_users_idx` (`id_user` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`subscriptions` (
  ...
  `users_premium_id_user` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id_subscription`),
  INDEX `fk_subscriptions_users_premium1_idx` (`users_premium_id_user` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`payments` (
  ...
  INDEX `fk_payments_users_premium1_idx` (`users_premium_id_user` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`paypal_details` (
  ...
  `username` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`payments_id_payment`),
  INDEX `fk_paypal_details_payments1_idx` (`payments_id_payment` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`playlists_active` (
  ...
  INDEX `fk_playlists_active_playlists1_idx` (`id_playlist_active` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`playlists_deleted` (
  `id_playlist_deleted` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `spotify`.`albums` (
  ...
  INDEX `fk_albums_artists1_idx` (`artists_id_artist` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`songs` (
  ...
  `albums_id_album` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id_song`),
...
CREATE TABLE IF NOT EXISTS `spotify`.`users_add_songs` (
  `users_id_user` INT UNSIGNED NOT NULL,
  `playlists_active_id_playlist_active` INT UNSIGNED NOT NULL,
  `songs_id_song` INT UNSIGNED NOT NULL,
  `date_added` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`users_id_user`, `playlists_active_id_playlist_active`, `songs_id_song`),
  INDEX `fk_users_has_playlists_active_playlists_active1_idx` (`playlists_active_id_playlist_active` ASC) ,
  INDEX `fk_users_has_playlists_active_users1_idx` (`users_id_user` ASC) ,
  INDEX `fk_add_sogs_idx` (`songs_id_song` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`users_follow_artists` (
  ...
  INDEX `fk_users_has_artists_artists1_idx` (`artists_id_artist` ASC) ,
  INDEX `fk_users_has_artists_users1_idx` (`users_id_user` ASC) ,
...
CREATE TABLE IF NOT EXISTS `spotify`.`artists_resemble_artists` (
  `artists_id_artist` INT UNSIGNED NOT NULL,
  `artists_id_artist1` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`artists_id_artist`, `artists_id_artist1`),
  INDEX `fk_artists_has_artists_artists2_idx` (`artists_id_artist1` ASC) ,
  INDEX `fk_artists_has_artists_artists1_idx` (`artists_id_artist` ASC) ,
